...
cover the routing + orchestration without needing the real engine or Java:
  ...
  - a scanned/unknown doc (no text yet) is SKIPPED and flagged -- the engine is
    never invoked, and the PDF is left untouched;
...
@pytest.mark.parametrize("route", [Route.SCANNED, Route.UNKNOWN])
def test_no_text_route_skips_engine_and_flags(route, tmp_path, monkeypatch):
    # A doc with no real text (scanned, or unknown-treated-as-scanned) can't be
    # tagged until OCR runs -- tag must skip it, flag it, and NOT touch the engine.
    calls = []
    stage = TagStage()
    ...
    result = stage.run(pdf, _sidecar(route))
